...
export const createProductController = async (req, res) => {
  try {
    ...
    const products = new prodModel({ ...req.fields, slug: slugify(name) });
    if (photo) {
...
export const productPhotoController = async (req, res) => {
  try {
    const product = await prodModel.findById(req.params.pid).select("photo");
...
export const productCountController = async (req, res) => {
  try {
    const total = await prodModel.find({}).estimatedDocumentCount();
    res.status(200).send({
      success: true,
      total,
    });
